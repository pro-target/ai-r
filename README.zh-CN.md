...
如今已经有少数几款跨智能体工具能读取不止一个智能体的历史
（`jazzyalex/agent-sessions`、`Dicklesworthstone/coding_agent_session_search`、
`hacktivist123/agent-session-resume`）。它们几乎都围绕**搜索与时间线**：找到某个
*会话*、翻阅历史。

`ai-r` 更进一步：它把**计划、意图和作者归属提取为现成的实体**，供你在其上构建
记忆。搜索找到的是文本——`ai-r` 回答的是**为什么**。技术上搜索工具也能从会话
文本里挖出一份计划，但它并不会把计划解析成单一、归一化的形态再交还给你——在
`ai-r` 里这正是主要面向的能力。

| 能力 | 单智能体查看器 | 跨智能体搜索工具 | `ai-r` |
|---|---|---|---|
| 读取 >1 个智能体的日志 | 否 | 是 | 是——Claude、Codex、OpenCode、Antigravity、Pi |
| 可编程接口 | 多为 GUI/TUI | 多为 TUI/CLI/应用 | **MCP + CLI + Python SDK** |
| 归属（编辑/命令 → 智能体 + 意图） | — | 部分 | 是——`find-file-edits` / `find-tool-calls` |
| 审计回放（无 git 重建一次会话的变更） | — | 罕见 | 是——`session_diff` |
| 计划提取（最终 vs 草稿，已归一化） | — | — | 是——`plan` |
| 范围 | 查看器 | 搜索 / 续接 / 记忆 | **只读提取内核** |

*竞品各列反映的是截至 2026-07 其公开文档的情况；凡某项能力不明确之处，我们
宁可低估也不夸大。*

我们刻意**不**在智能体数量、速度或 TUI 的丰富度上竞争。`ai-r` 的切入点在于
提取“为什么”以及供机器消费的结构化实体。

## 实践检验

`ai-r` 已经在读取它自己的开发历史——覆盖全部五个智能体。真实的工具运行在它
之上（它们独立存在，构建于其只读 API 之上）：

- **auditor**——一个全新的智能体冷静地核查上一个智能体究竟做了什么、决定了
  什么。这抓到了那些在计划上悄悄撒谎的智能体。
- **summarizer**（`export rounds`）——把一次会话渲染成一份现成的交接文档。
- **ai-local-reader**——一个只读技能：跨全部智能体从磁盘审计过往会话。

这些工具位于工作流一侧，在本仓库之外。`ai-r` 本身只读取并返回数据。
...
- 验收场景（104 个 e2e）—— [`docs/scenarios.md`](./docs/scenarios.md)
