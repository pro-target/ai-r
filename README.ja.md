...
**決定論的な読み取りパス（read path）: 抽出は LLM を一切呼び出さず、外部へのネットワークリクエストも行いません。オプションのセマンティック再ランク（re-rank）もローカル完結です（LLM ではなくエンベディング（embeddings））。**
...
- 受け入れシナリオ（95 個の e2e）— [`docs/scenarios.md`](./docs/scenarios.md)
