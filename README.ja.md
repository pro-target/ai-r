...
**共有 streamable-http トランスポート**を有効化します。既定では各エージェントが
自前の `ai-r-mcp` を stdio 上で起動します — マルチエージェントのファンアウト下では
これは N 個のプロセスとなり、それぞれがコールドキャッシュでコーパスを再スキャン
します（RAM 枯渇の実測された原因）。`AI_R_MCP_TRANSPORT=http` を指定すると、
スウォームの代わりに localhost（既定 `127.0.0.1:8756`）上の単一の**ウォームな
サーバー**をすべてのエージェントが共有します。`packaging/systemd/` の systemd
ユニットはアイドル時の自己終了を伴うソケットアクティベーションを追加します —
プロセスは負荷がかかっているときだけ存在します。バインドはループバック専用かつ
**フェイルクローズド（fail-closed）**です: localhost 以外の `AI_R_MCP_HOST` は、
オペレーターが明示的に `AI_R_MCP_ALLOW_REMOTE=1` を設定するまで拒否されます
（トランスクリプトはシークレットを含み、トークンなしで配信されるため）。その他の
つまみ: `AI_R_MCP_PORT`、`AI_R_MCP_IDLE_SEC`（アイドル自己終了のしきい値）、
`AI_R_HAYSTACK_CACHE_MAX`（検索キャッシュの上限）。完全にオプションです: これが
なくても stdio モードは従来どおり動作します。
...
- 受け入れシナリオ（84 個の e2e）— [`docs/scenarios.md`](./docs/scenarios.md)
