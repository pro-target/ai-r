...
**エージェントは「完了」と報告した。それを突き合わせる相手がいない。**

`ai-r` は 5 つのコーディングエージェントいずれのセッション履歴も読み取り、
新鮮なエージェントに、`git` では答えられないことを冷静に検証させます。

- 嘘をついていないか、何かを壊していないか — 約束を守ったか、危険なものを
  実行していないか（実行したなら取り消したか）、実際に何を変えたか、いくら
  かかったか。
- なぜそうしたのか — どのプランのもとで、どんな意図で、そしてその編集の背後に
  あった手は誰のものか。

> 私たち自身のコーパス — 20 以上のプロジェクトにわたる 5 つのエージェントの
> 1600 以上のセッション — で、こうして 312 件の危険なコマンド（`rm -rf`、
> `curl|sh`、`git push --force`）を見つけました。エージェント自身が気づいて
> 取り消したのは 2 件。残る 310 件は黙って実行され — `git` はそれを見せません。

`git` はコードに**何が**入ったかを示します。`ai-r` は、エージェントがそこに
**どう**たどり着いたかを信頼できるかどうかを示します。読み取り専用: LLM 呼び出し
なし、ネットワークなし。
...
- **新鮮な目でセッションを監査。** 空のコンテキストを持つ新しいエージェントが、
  ...
  見落としたか。ある実際の実行では、この方法で対話をまとめてレビューし、
  タスクは完了したが**プランニングで誤導した**エージェントを捉えました —
...
いまや複数のエージェントの履歴を読むエージェント横断ツールがいくつか
存在します（`jazzyalex/agent-sessions`、
`Dicklesworthstone/coding_agent_session_search`、
`hacktivist123/agent-session-resume`）。そのほとんどは**検索とタイムライン**が
主眼です。*セッション*を見つけ、履歴をスクロールします。

`ai-r` はさらに深く踏み込みます。**プラン・意図・作者性を、そのまま使える
エンティティ**として取り出し、その上にメモリを構築できます。検索はテキストを
見つけます — `ai-r` は**なぜ**に答えます。技術的には検索ツールでもセッションの
テキストからプランを掘り出すことはできますが、単一の正規化された形状にパース
して返してはくれません — `ai-r` ではそれが主要なサーフェスです。

| 能力 | 単一エージェントのビューア | エージェント横断の検索ツール | `ai-r` |
|---|---|---|---|
| 複数エージェントのログを読む | いいえ | はい | はい — Claude、Codex、OpenCode、Antigravity、Pi |
| プログラム的なサーフェス | ほぼ GUI/TUI | ほぼ TUI/CLI/アプリ | **MCP + CLI + Python SDK** |
| 作者特定（編集/コマンド → エージェント + 意図） | — | 部分的 | はい — `find-file-edits` / `find-tool-calls` |
| 監査リプレイ（git なしでセッションの変更を再構築） | — | まれ | はい — `session_diff` |
| プラン抽出（最終 vs ドラフト、正規化済み） | — | — | はい — `plan` |
| スコープ | ビューア | 検索 / 再開 / メモリ | **読み取り専用の抽出コア** |

*競合の列は 2026-07 時点の公開ドキュメントを反映しています。能力が不明確な場合は
過大に主張せず控えめに記しています。*

私たちは意図的に、エージェントの網羅数・速度・TUI の充実度では**競いません**。
`ai-r` の楔（くさび）は、「なぜ」と機械が消費できる構造化エンティティを取り出す
ことにあります。

## 実践で実証済み

`ai-r` はすでに自身の開発履歴を — 5 つのエージェントすべてを横断して — 読んで
います。実際のツールがその上で動いています（それらは別に存在し、読み取り専用の
API の上に載っています）:

- **auditor** — 新鮮なエージェントが、前のエージェントが実際に何をして何を
  決めたかを冷静にチェックします。これはプランについてこっそり偽ったエージェント
  を捉えました。
- **summarizer**（`export rounds`）— セッションを、そのまま使える引き継ぎ
  ドキュメントに描き出します。
- **ai-local-reader** — 読み取り専用のスキル: すべてのエージェントを横断して
  過去のセッションをディスクから監査します。
...
- 受け入れシナリオ（104 個の e2e）— [`docs/scenarios.md`](./docs/scenarios.md)
