...
Необязательное дополнение — `http`: `AI_R_EXTRAS=http bash install.sh`
(или `pip install "ai-r[http]"`) добавляет [uvicorn](https://www.uvicorn.org) и
включает **общий streamable-http транспорт**. По умолчанию каждый агент
поднимает свой `ai-r-mcp` по stdio — под многоагентным веером это N процессов,
каждый со своим холодным кэшем, перечитывающих корпус (измеренная причина
исчерпания RAM). С `AI_R_MCP_TRANSPORT=http` вместо роя работает **один тёплый
сервер** на localhost (по умолчанию `127.0.0.1:8756`), общий для всех агентов;
systemd-юниты в `packaging/systemd/` дают socket-activation с само-выходом по
простою — процесс существует только под нагрузкой. Bind — только loopback и
**с закрытием по умолчанию (fail-closed)**: не-localhost `AI_R_MCP_HOST`
отклоняется (транскрипты содержат секреты и отдаются без токена), пока оператор
явно не выставит `AI_R_MCP_ALLOW_REMOTE=1`. Прочие ручки: `AI_R_MCP_PORT`,
`AI_R_MCP_IDLE_SEC` (порог само-выхода по простою), `AI_R_HAYSTACK_CACHE_MAX`
(потолок кэша поиска). Полностью опционально: без него stdio-режим работает как
раньше.
