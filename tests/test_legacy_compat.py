"""Tests for ``ai_r.legacy_compat`` — the backward-compat shim.
...
* If ``ai-r`` is missing → return ``None`` (caller falls back).
* If any requested flag has no ``ai-r`` equivalent → return ``None``.
* Otherwise → run ``ai-r`` and propagate its exit code.

We monkey-patch ``subprocess.run`` and the availability helper so the
tests are hermetic — no real ``ai-r`` binary is invoked.
"""
...
from ai_r import legacy_compat
...
def _available(monkeypatch, on: bool = True) -> None:
    monkeypatch.setattr(legacy_compat, "is_ai_r_available", lambda: on)
...
def test_is_ai_r_available_uses_path(monkeypatch):
    monkeypatch.setattr(legacy_compat.shutil, "which", lambda name: "/usr/bin/x" if name == "ai-r" else None)
    assert legacy_compat.is_ai_r_available() is True

    monkeypatch.setattr(legacy_compat.shutil, "which", lambda name: None)
    assert legacy_compat.is_ai_r_available() is False
...
def test_get_latest_context_returns_none_when_ai_r_missing(monkeypatch, clean_argv):
...
def test_get_latest_context_id_with_unsupported_agent_falls_back(monkeypatch, clean_argv):
    """ROO has no ``ai-r`` equivalent → fallback."""
    _available(monkeypatch)
...
def test_get_latest_context_limit_falls_back(monkeypatch, clean_argv):
    """``ai-r list`` has no ``--limit``; legacy must be used to
...
def test_get_latest_context_id_with_agent_invokes_ai_r(monkeypatch, clean_argv):
    _available(monkeypatch)
    calls: List[List[str]] = []
    ...
    assert rc == 0
    assert calls == [["ai-r", "read", "--agent", "opencode", "ses_abc"]]
...
def test_get_latest_context_agent_only_invokes_list(monkeypatch, clean_argv):
    ...
    rc = legacy_compat.run_legacy_get_latest_context()
    assert rc == 0
    assert calls == [["ai-r", "list", "--agent", "claude"]]
...
def test_get_latest_context_all_agents_invokes_list(monkeypatch, clean_argv):
    ...
    rc = legacy_compat.run_legacy_get_latest_context()
    assert rc == 0
    assert calls == [["ai-r", "list"]]


def test_get_latest_context_no_args_falls_back(monkeypatch, clean_argv):
    """No flags → legacy uses ``CURRENT_AGENT`` env var; ai-r's
...
def test_audit_search_falls_back(monkeypatch, clean_argv):
    ...
    ai-r — different semantics, must fall back."""
    _available(monkeypatch)
...
def test_audit_agent_only_invokes_list(monkeypatch, clean_argv):
    ...
    assert rc == 0
    assert calls == [["ai-r", "list", "--agent", "opencode"]]
...
def test_audit_id_with_agent_and_fuzzy_invokes_read(monkeypatch, clean_argv):
    ...
    rc = legacy_compat.run_legacy_agent_audit()
    assert rc == 0
    assert calls == [["ai-r", "read", "--agent", "claude", "46d7b4fc"]]
...
def test_audit_id_with_fuzzy_invokes_cross_agent_read(monkeypatch, clean_argv):
    ...
    rc = legacy_compat.run_legacy_agent_audit()
    assert rc == 0
    assert calls == [["ai-r", "read", "46d7b4fc"]]
...
def test_audit_returns_none_when_ai_r_missing(monkeypatch, clean_argv):
    _available(monkeypatch, on=False)
    _set_argv()
    assert legacy_compat.run_legacy_agent_audit() is None
...
def test_run_ai_r_propagates_exit_code(monkeypatch):
    ...
    assert legacy_compat._run_ai_r(["list"]) == 42
    fake.assert_called_once()
    args, kwargs = fake.call_args
    assert args[0] == ["ai-r", "list"]
    assert kwargs["stdout"] is sys.stdout
    assert kwargs["stderr"] is sys.stderr
    assert kwargs["check"] is False


def test_run_ai_r_handles_missing_binary(monkeypatch):
    def boom(*_args, **_kwargs):
        raise FileNotFoundError("ai-r")

    monkeypatch.setattr(legacy_compat.subprocess, "run", boom)
    assert legacy_compat._run_ai_r(["list"]) == 127
