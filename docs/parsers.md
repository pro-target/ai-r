...
and Google-style negative prefixes (`-claude` always excludes). Boolean
behaviour is selected by the separate `operator` parameter: `AND`
(default), `OR`, or `NOT`. Words `AND`, `OR`, and `NOT` inside the query
are literal search terms. When a body match is found, the result carries a
`snippet` field (up to 200 chars) so the caller can show "what was found"
without a second `read_session` round-trip. See
[README.md § Search operators](../README.md#search-operators) for the full
table.
...
def read_messages(uuid: str, base_dir: str | None = None) -> list[Message]:
    root = Path(base_dir) if base_dir else _home()
    messages = root / uuid / "messages.jsonl"
    if not messages.is_file():
        raise FileNotFoundError(f"gemini-cli session not found: {uuid}")
    out: list[Message] = []
...
## Step 4 — register in the canonical parser registry

The CLI and MCP use the canonical registry in `src/ai_r/parsers/__init__.py`.
Update `AgentName`, import the new module there, and add it to `PARSERS`.
CLI choices and MCP dispatch pick it up from that registry.
