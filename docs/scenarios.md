...
Full spec: [docs/scenarios.md](docs/scenarios.md) — 97 LLM-executed end-to-end scenarios validating the whole public surface on a real vault. Kept in English as language-neutral, executable test specs.
...
| [`attention` (preset)](#attention-preset) | 3 | One call finds rushed-approval decision gates (F4.4) via a baked chain — ONE `query(type=tool_call)` scan filtered to the interactive gate tools (Claude `ExitPlanMode` plan approval, `AskUserQuestion`; an auto-approved edit/delete lands at machine speed and is deliberately excluded — only these gates carry a real human-decision timestamp) → the user's answer located at the **message** level (`tool_result` correlated by `tool_use_id`, NOT a `user_turn` event) → gap `answer.ts − gate.ts` vs a reading-speed model (`content_chars / gap_sec` against `AVG_CPS≈25`; RED at `ratio ≥ 4` or a near-instant sub-`floor_sec` answer to ≥`min_chars` content, AMBER at `ratio ≥ 2`, else `null`); each record carries the query event `id` (context on-demand via `relative_to`), `gate`/`tool`, `content_chars` (`null` = unmeasured → floor-only), `gap_sec`, `required_cps`/`ratio`, `severity` and a coarse `reaction`; `count`/`red_count`/`amber_count`/`by_gate` reflect the FULL match set independent of `limit`; `gate`/`severity` filters fail loud on unknown values; emitted preview/title redacted by default; zero gates → `diagnostics`; effective thresholds echoed in `params`; documented caveats: streaming/self-authorship not credited (a signal to review, not proof), AFK inflates the gap (under-flags, never fabricates), clock-skew gaps dropped. |
...
The F4.4 preset: rushed-approval pacing audit, one call. A baked chain over the existing core (never a
second engine): ONE `query(type=tool_call)` scan filtered to the interactive gate tools supplies the
candidates; the pacing verdict is pure arithmetic on transcript timestamps (zero LLM). Only the plan
(`ExitPlanMode`) and question (`AskUserQuestion`) gates participate — they are the only approvals that
carry a genuine human-decision timestamp (an auto-approved edit/delete lands at machine speed).

<details>
<summary>Show 3 scenarios (ATT-1…ATT-3)</summary>

### ATT-1 — rushed plan approval surfaces as a red gate (record shape) `[needs-real-vault]`
- **Function:** `attention`
- **Steps:** `mcp__ai-r__attention(severity="all", limit=10)`; pick a `gate="plan"` record; cross-check its `id` via `mcp__ai-r__query(relative_to="<id>", direction="next")` and `mcp__ai-r__read_session(<session_id>)`.
- **Expected:** `{gates:[…], count, red_count, amber_count, by_gate, truncated, params}`; every record carries `id` (a query event id), `agent`, `session_id`, `ts`, `gate` (`plan`/`question`), `tool`, `gap_sec`, `severity` (`red`/`amber`/`null`) and a `reaction` (`kind`+capped preview); a plan approved in a few seconds on a multi-KB plan body shows `severity="red"` with `ratio ≫ 4`; records are chronological (ts ascending); `params` echoes the effective thresholds.

### ATT-2 — a slow, read approval is NOT flagged; `severity`/`gate` filters `[needs-real-vault]`
- **Function:** `attention`
- **Steps:** `mcp__ai-r__attention()` (default `flagged`), `mcp__ai-r__attention(severity="red")`, `mcp__ai-r__attention(gate="question")`.
- **Expected:** a gate answered after a genuine pause (`ratio < 2`) is absent from `flagged` but present under `severity="all"`; `severity="red"` ⊆ `flagged`; `gate="question"` keeps only `AskUserQuestion` gates; `count`/`red_count`/`amber_count` reflect the full matched set independent of `limit`.

### ATT-3 — fail-loud validation + empty-result diagnostics `[hermetic-ok]`
- **Function:** `attention`
- **Steps:** `mcp__ai-r__attention(gate="edit")`, `mcp__ai-r__attention(severity="high")`, `mcp__ai-r__attention(limit=-1)`; then a valid call on an empty/filtered-to-zero corpus.
- **Expected:** each invalid call returns `{"error":"invalid_argument", "message": …}` naming the bad parameter; the empty corpus returns `count=0` + `diagnostics` (never a crash).
