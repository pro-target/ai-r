...
| `read_session` | 5 | Reads one session into the compact `{role, content}` projection with metadata + pagination echo; `offset`/`limit` page a stable ordered list, `total` invariant across slices; `agent` is **optional** — an id resolves across every parser, a rare cross-agent id collision returns a `candidates` list (not an error), a miss names `agents_scanned`; `with_tokens=true` (F3.3) adds `summary.tokens` (exact-or-estimate + a `categories` estimate sub-block — text/thinking/tool_input/tool_result summing to `total`, always self-labeled `source="estimate"`, tiers never merged, empty transcript → `null`) plus per-message EXACT `tokens` blocks where the format records per-message usage (Claude deduped per API call before pagination, OpenCode, Pi; Codex/Antigravity/user turns carry no key — absent, not null); integers-and-labels only → outside redaction; default `false` is byte-identical to the historical output. |
...
### READ-4 — `with_tokens=true`: session `categories` estimate + per-message exact blocks (Claude) `[needs-real-vault]`
- **Function:** `read_session` (F3.3 follow-up `with_tokens`)
- **Goal:** A Claude session yields `summary.tokens` with an exact-or-estimate outer block AND a `categories` estimate sub-block whose four surfaces sum to the total, plus per-message EXACT `tokens` blocks on assistant messages, deduplicated per API call.
- **Preconditions:** A Claude session that records per-call `message.usage` (multiple assistant API calls, ideally with streamed duplicates sharing a `(message.id, requestId)`). `[needs-real-vault]`.
- **Steps:** `mcp__ai-r__read_session(uuid="<uuid>", agent="claude", with_tokens=true)`; inspect `summary.tokens` and per-message `tokens`.
- **Expected:** `summary.tokens` is the `session_tokens` exact-or-estimate shape and now carries `categories` = `{text, thinking, tool_input, tool_result, total, source:"estimate", estimator}` — the four surface counts sum to `categories.total`, and `categories` self-labels `source="estimate"` + `estimator` even when the outer block is exact (tiers never merged); an empty transcript would give `categories: null`. Assistant message entries where the format records per-message usage carry an EXACT `tokens` block; the same `(message.id, requestId)` API call is counted ONCE (dedup on absolute positions BEFORE pagination), never double-counted across streamed duplicates; user turns carry NO `tokens` key (absent, not null). Every emitted token value is an integer + ai-r label (no raw session text).
- **Pass criteria:** GO when `sum(categories.text+thinking+tool_input+tool_result) == categories.total`, `categories.source == "estimate"` regardless of the outer tier, per-message exact blocks appear only on assistant messages the format records usage for and each API call is deduped (no inflated per-message total), and no `tokens` key leaks onto user turns. A `categories` tier merged into an exact outer block, or a double-counted streamed call, is NO-GO.

### READ-5 — default byte-identical + Codex session-block-only (no per-message keys) `[hermetic-ok]`
- **Function:** `read_session` (F3.3 follow-up `with_tokens`)
- **Goal:** The default `with_tokens=false` is byte-identical to the historical output, and a Codex session (cumulative-only usage) yields the session `tokens` block WITHOUT any per-message `tokens` keys.
- **Preconditions:** `[hermetic-ok]` — seed under `AI_R_HOME` (a) any small Claude session and (b) a Codex session whose usage is recorded only cumulatively (`token_count`), with no per-message usage.
- **Steps:** `mcp__ai-r__read_session(uuid="<uuid>", agent="claude")` (default) and the same call with `with_tokens=false` — compare; then `mcp__ai-r__read_session(uuid="<codex-uuid>", agent="codex", with_tokens=true)`; inspect message entries.
- **Expected:** The default call and the explicit `with_tokens=false` call are byte-identical, and neither carries a `summary.tokens` block nor any per-message `tokens` key (historical shape unchanged). The Codex `with_tokens=true` call carries `summary.tokens` (exact-or-estimate + `categories`) but NO message entry carries a `tokens` key — Codex is cumulative-only, so per-message exact blocks are absent (not null), exactly like Antigravity and user turns.
- **Pass criteria:** GO when omitting `with_tokens` (or passing `false`) reproduces the pre-feature output byte-for-byte, and the Codex session exposes only the session-level block with zero per-message `tokens` keys. A per-message `tokens` key on a Codex message, or any diff in the default output, is NO-GO.
