...
- **Goal:** A Claude session yields `summary.tokens` (flat exact-or-estimate) AND `summary.component_tokens` — a per-component estimate over ai-r's existing event taxonomy whose scalars + `tool_call` map sum to the total, plus per-message EXACT `tokens` blocks on assistant messages, deduplicated per API call.
...
- **Steps:** `mcp__ai-r__read_session(uuid="<uuid>", agent="claude", with_tokens=true)`; inspect `summary.tokens`, `summary.component_tokens` and per-message `tokens`.
- **Expected:** `summary.tokens` is the flat `session_tokens` exact-or-estimate block; `summary.component_tokens` = `{user_turn, assistant_turn, thinking, plan, tool_call: {<tool_kind>: n}, total, source:"estimate", estimator}` over ai-r's existing taxonomy (reused classifiers — `resolve_tool`, the plan-signal detector, the user/assistant role — not a second classifier); `total == user_turn + assistant_turn + thinking + plan + sum(tool_call.values())`; a plan-authoring call (`ExitPlanMode` / `Write plans/*.md`) counts under `plan`, NEVER under `tool_call` (no double count); every other call's `input` + its correlated `tool_result` content is bucketed by `tool_kind`; ONE estimator drives every surface, so `component_tokens.source == "estimate"` even when `summary.tokens` is exact (tiers never merged); an empty transcript would give `component_tokens: null`. Assistant message entries where the format records per-message usage carry an EXACT `tokens` block; the same `(message.id, requestId)` API call is counted ONCE (dedup on absolute positions BEFORE pagination), never double-counted across streamed duplicates; user turns carry NO `tokens` key (absent, not null). Every emitted token value is an integer + ai-r label (no raw session text).
- **Pass criteria:** GO when `component_tokens.total == sum(user_turn, assistant_turn, thinking, plan) + sum(tool_call.values())`, a plan-authoring call lands under `plan` and not `tool_call`, `component_tokens.source == "estimate"` regardless of the `summary.tokens` tier, per-message exact blocks appear only on assistant messages the format records usage for and each API call is deduped (no inflated per-message total), and no `tokens` key leaks onto user turns. A plan-authoring call double-counted into `tool_call`, a `component_tokens` tier merged into an exact `summary.tokens`, or a double-counted streamed call, is NO-GO.
...
- **Goal:** The default `with_tokens=false` is byte-identical to the historical output; `include_subagents=true` on a parent folds its subagents into a `summary.subagent_rollup`; and a Codex session (cumulative-only usage) carries NO per-message `tokens` keys.
