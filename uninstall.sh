...
#   - ~/.agents/skills/ai-r/SKILL.md when it matches the bundled template
...
Usage:
  bash uninstall.sh           Remove symlinks + 4 agent config entries + Pi skill
...
# Pi skill (remove only if it still matches the bundled installer template)
skill="$HOME/.agents/skills/ai-r/SKILL.md"
skill_src="$REPO_DIR/install/pi/skills/ai-r/SKILL.md"
if [[ -f "$skill" && -f "$skill_src" ]]; then
    if cmp -s "$skill" "$skill_src"; then
        backup_file "$skill"
        rm -f "$skill"
        rmdir "$HOME/.agents/skills/ai-r" 2>/dev/null || true
        log "Pi:        removed ai-r skill"
    else
        warn "Pi skill differs from bundled template — leaving $skill"
    fi
else
    log "Pi:        ai-r skill not present"
fi
